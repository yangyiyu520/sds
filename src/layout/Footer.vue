          ...
          >
            <!-- <transition name="el-fade-in-linear">
              <div class="mask-playicon" v-show="showMask" style="cursor: pointer; background: transparent">
                <span
                  :class="!isShowSongDetail ? 'iconfont icon-xiangshangjiantou' : 'iconfont icon-down-arrow'"
                ></span>
              </div>
            </transition> -->
...
export default {
  ...
  methods: {
    ...
    changeState(isPlay) {
      ...
      if (isPlay) {
        this.$store.commit("showSongDetail", isPlay);
      }
    },
    // 播放音乐
    playMusic() {
    ...
    orderPlay(type) {
      ...
      //调用函数根据对应的歌曲信息获取
      this.getSongUrlBy(this.playingList[toggleIndex]);
    },
    //.2随机模式
    randomPlay() {
      // 获得一个随机索引号
