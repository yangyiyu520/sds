  ...
  <div class="theme">
    <div class="theme-item" v-for="(item, index) in themeList" :key="index" @click="changeTheme(item, index)">
      <div :style="{ background: item.themeColor }" class="bg">
        <span class="iconfont icon-duihao" v-show="currentIndex === index"></span>
      </div>
      <div>{{ item.title }}</div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      themeList: [
        {
          title: "蓝色",
          mycompColor: "#5292fe", //header盒子
          themeColor: "#409eff", //轮播图
          themeRgba: "rgba(64, 158, 255, 0.8)", //播放按钮
  ...
  created() {
    // 设置主题色
    let currentTheme = this.getItem("currentTheme");
    if (currentTheme) {
